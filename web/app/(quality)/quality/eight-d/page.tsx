...
export default function QualityEightDPage() {
  const [incidentId, setIncidentId] = useState("INC-2026-0412");
  const [sections, setSections] = useState<EightDSection[]>([]);
  const [rootCauseGraph, setRootCauseGraph] = useState<CytoscapeGraph>({
    nodes: [],
    edges: [],
  });
  const [open, setOpen] = useState<Set<string>>(new Set(["D1", "D4"]));
  const [loading, setLoading] = useState(false);

  async function submit(e: React.FormEvent) {
    e.preventDefault();
    setLoading(true);
    const r = (await api.eightD(incidentId)) as {
      sections?: EightDSection[];
      root_cause_graph?: CytoscapeGraph;
    };
    setSections(r.sections ?? []);
    setRootCauseGraph(r.root_cause_graph ?? { nodes: [], edges: [] });
    setLoading(false);
  }
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div>
        <h1 className="font-bold text-xl mb-3">8D 보고서 (Quality)</h1>
      ...
      <div>
        <h2 className="font-bold text-lg mb-2">근본 원인 그래프 (D4)</h2>
        <CytoscapeView graph={rootCauseGraph} />
      </div>
    </div>
  );
}
