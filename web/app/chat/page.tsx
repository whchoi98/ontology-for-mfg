...
const SUGGESTIONS: Record<string, string[]> = {
  buyer:    ["COMP-CAP-2023 부품의 REACH SVHC 위반 가능성?", "현재 납기 위험이 가장 높은 협력사는?"],
  engineer: ["AEC-Q100 Grade 0 기준 대체 MCU 추천해줘", "PFAS 미함유 부품 목록 조회"],
  quality:  ["INC-2026-0412 인시던트 근본원인 분석", "최근 3개월 RoHS 위반 이력"],
  scm:      ["IRA 2026 관세 변경 시 lane reroute 시뮬레이션", "멕시코 공장 공급망 위험"],
  plant:    ["PLANT-KR-01 예지보전 알람 요약", "Scope 3 배출 절감 방안"],
};
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {(SUGGESTIONS[active] ?? SUGGESTIONS.buyer).map((s) => (
              <button
                key={s}
                onClick={() => send(s)}
                className="text-xs px-2 py-1 rounded bg-ink-800 border border-ink-700 text-ink-300 hover:border-accent-500/50 transition"
              >
                {s}
              </button>
            ))}
